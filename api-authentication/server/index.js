...
app.post('/api/auth/sign-in', (req, res, next) => {
  ...
  db.query(sql, params)
    .then(result => {
      ...
      const payloaded = { userId: result.rows[0].userId, username: username };
      const results = result.rows[0].hashedPassword;

      const verifying = argon2.verify(results, password);
      verifying.then(pwcheck => {

        if (pwcheck) {
          const token = jwt.sign(payloaded, process.env.TOKEN_SECRET);
          res.status(200).json({ payloaded, token });
        } else {
          throw new ClientError(401, 'Invalid Login');
        }
      }
